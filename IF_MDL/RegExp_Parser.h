		...
		>> select_p(rpsMain)
			...
			.case_p(
				select_p(rpsMainMetaChar)
					...
					.case_p(action_p(rpsCharClass, ch_p('[')) >> call_p(2) >> ch_p(']'))
					.case_p(not_p("?*+)"))
...
private:
	...
	character_parallel_regexp<target_iterator, target_compare> fchar_class;
...
public:
	...
	void add_char_class(void)
	{
		if (fnot_char_set)
		{
			fchar_class.classter_items();
			fregexp >>= fchar_class;
		}
		else
			fregexp >>= fchar_class;
	}
	void add_char_class_char(void)
	{
		fchar_class += character_parallel_regexp<target_iterator, target_compare>(fchar, fnot_char_set);
		//if (fnot_char_set)
		//	fchar_class >>= character_parallel_regexp<target_iterator, target_compare>(fchar, fnot_char_set);
		//else
		//	fchar_class |= character_parallel_regexp<target_iterator, target_compare>(fchar, fnot_char_set);
	}
	void clear_char_class(void)
	{
		fchar_class.clear();
	}
	...
	void push_char_class(void)
	{
		for_stack.push_back(fregexp);
		fregexp.clear();
	}
	void back_char_class(void)
	{
		if (for_stack.size())
		{
			push_or();
