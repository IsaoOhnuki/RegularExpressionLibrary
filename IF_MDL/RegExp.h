...
public:
	...
	public:
		...
		bool is_matchs()
		{
			return fmatch_stack.size() > 0 && fmatch_stack.back().size() > 0;
		}
...
public:
	...
	bool is_null(void) const { return fbase.size() == 1; }
	void clear(void)
	{
	...
	: public iregexp_base<target_iterator, target_compare>
{
	...
	protected:
		...
		virtual bool _syntax(typename iregexp_item_tree::const_iterator src, const target_iterator vb, target_iterator &vp, const target_iterator ve, bool backtracking)
		{
			...
			{
				ret = ftarget.size() > 0;
				for (std::vector<target_range>::const_iterator i = ftarget.begin(); ret && i < ftarget.end(); ++i)
				{
					ret = target_range_size()(*i) > 0 && vp < ve && !target_find()(i->first, i->second, vp);
				}
			}
			else
			{
				...
				for (std::vector<target_range>::const_iterator i = ftarget.begin(); !ret && i < ftarget.end(); ++i)
				{
					ret = target_range_size()(*i) > 0 && vp < ve && (fnot ^ target_find()(i->first, i->second, vp));
				}
			}
			if (ret)
				++vp;
	...
	public:
		regexp_parallel_item() : fnot(false) {}
		...
		virtual void get_type_string(std::wstring& str) const
		{
			...
			for (std::vector<target_range>::const_iterator i = ftarget.begin(); i < ftarget.end(); ++i)
			{
				if (*((wchar_t*)&*i->first) < 0x20)
					break;
				str.append((wchar_t*)&*i->first, target_range_size()(*i));
			}
			str += L"')";
		}
		std::vector<target_range>* get_character()
		{
			return &ftarget;
		}
		...
		void append_character(const std::vector<target_range> v)
		{
			for (std::vector<target_range>::const_iterator i = v.begin(); i < v.end(); ++i)
			{
				ftarget.push_back(*i);
			}
		}
...
public:
	...
	{
		add_item(begin_item(), new regexp_parallel_item());
	}
	character_parallel_regexp(const target_range& v, bool not = false)
	{
		add_item(begin_item(), (iregexp_item*)new regexp_parallel_item(v, not));
	}
	...
	character_parallel_regexp& operator += (const character_parallel_regexp& v)
	{
		if (v.is_null())
			return *this;
		if (is_null())
			return (*this = v);
		regexp_parallel_item *s = dynamic_cast<regexp_parallel_item*>(&**begin_item().first_child());
		regexp_parallel_item *s1 = dynamic_cast<regexp_parallel_item*>(&**v.begin_item().first_child());
		s->append_character(*s1->get_character());
		return *this;
	}
};
